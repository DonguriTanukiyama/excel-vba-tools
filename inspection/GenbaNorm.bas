...
' 同じフォルダの genba_norm.py（Python。rag-ja からの写し）と同じ規則で書いてある。
' 結果は同じフォルダの compare.py で、Python 版の出力と1セルずつ突き合わせる。
'
' 使い方
...
' ・名前に予約語を使わない。VBA は大文字と小文字を区別しないので、eNum は Enum と同じに読まれて
'   「構文エラー」になる（実際に1回引っかかった）。Excel に取り込む前に tools/check_bas.py で調べる。
'
' ============================================================
' 既知の限界
' ============================================================
...
Public Sub NormalizeFile(ByVal path As String)
    Dim wbOut As Workbook, res As Variant, n As Long, nCheck As Long
    Dim r As Long, k As Long, nDate As Long, tally(0 To 5) As Long
    Dim outPath As String, alerts As Boolean, msg As String

    alerts = Application.DisplayAlerts
    On Error GoTo Fail

    res = ReadTable(path, n, nCheck)
    For r = 1 To n
        If res(r, 2) <> "読めない" Then nDate = nDate + 1
        For k = 1 To nCheck
            tally(KindIndex(res(r, 5 + k))) = tally(KindIndex(res(r, 5 + k))) + 1
        Next
    Next

    ' 書き出す。**先に文字列書式にする**（日付や数値に化けさせない）
    ' 範囲は 5 + nCheck 列。配列の右側（原文・備考）は範囲の外なので書かれない。CSV の列は前と同じ
...
Fail:
    ...
    MsgBox "止まりました。" & vbCrLf & vbCrLf & msg, vbExclamation, "GenbaNorm"
End Sub

' 点検表を読み、整えた表を配列で返す。報告書を作る GenbaReport もここを呼ぶ。
' 0 行目は見出し。1 行目から n 行目までが点検の行。nCheck は点検欄の数。
' 列: 行 / 日付 / 担当 / 部署 / 汚泥厚 / 点検欄の区分 × nCheck / 点検欄の原文 × nCheck / 備考
' 元のファイルは読み取り専用で開き、読み終えたら閉じる。

Public Function ReadTable(ByVal path As String, ByRef n As Long, ByRef nCheck As Long) As Variant
    Dim wbSrc As Workbook, ws As Worksheet
    Dim lastRow As Long, lastCol As Long, r As Long, c As Long, k As Long
    Dim dy As Long, dm As Long, stem As String, noteCol As Long
    Dim checkCols() As Long, heads() As String
    ...
    For c = 1 To lastCol
        ...
        If InStr(heads(c), "掻き取り") > 0 Or InStr(heads(c), "異音") > 0 Or _
           InStr(heads(c), "発泡") > 0 Or InStr(heads(c), "色") > 0 Then
            nCheck = nCheck + 1
            checkCols(nCheck) = c
        ElseIf InStr(heads(c), "備考") > 0 And noteCol = 0 Then
            noteCol = c
        End If
    Next
    ' 点検欄の列が1つも無ければ、点検表ではない（ひな形などを選んだ）。何も作らずに止める
    If nCheck = 0 Then
        Err.Raise vbObjectError + 515, "ReadTable", _
                  "点検欄（掻き取り・異音・発泡・色）の列が見つかりません。点検表のファイルか確かめてください: " & _
    ...
    For r = 3 To lastRow
        If RowHasValue(ws, r, lastCol) Then
            ...
            For k = 1 To nCheck
                res(n, 5 + k) = Classify(CellVal(ws, r, checkCols(k)))
                res(n, 5 + nCheck + k) = StripW(CStr(CellVal(ws, r, checkCols(k))))
            Next

            If noteCol > 0 Then
                res(n, 6 + 2 * nCheck) = StripW(CStr(CellVal(ws, r, noteCol)))
            Else
                res(n, 6 + 2 * nCheck) = ""
            End If
        End If
...
Fail:
    ' 開いた点検表を閉じてから、呼び出し元にエラーを返す
    errNo = Err.Number: errText = Err.Description
    On Error Resume Next
    If Not wbSrc Is Nothing Then wbSrc.Close SaveChanges:=False
    On Error GoTo 0
    Err.Raise errNo, "ReadTable", errText
End Function
